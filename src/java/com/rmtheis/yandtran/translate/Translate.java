...
public final class Translate extends YandexTranslatorAPI {
  ...
  public static String execute(final String text, final Language from, final Language to) throws Exception {
    ...
    Log.info("Yandex URL = [ "+ SERVICE_URL + params + " ]");
    String str = retrievePropArrString(url, TRANSLATION_LABEL).trim();
    Log.info("Yandex responce = [ "+ str + " ]");
    return str;//retrievePropArrString(url, TRANSLATION_LABEL).trim();
  }
